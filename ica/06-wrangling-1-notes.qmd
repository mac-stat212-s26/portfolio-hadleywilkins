...
diamonds |> 
  filter(price <= 600 & price >= 500) #inclusive requires =
...
diamonds |> 
  filter(cut == "Fair" & price > 30000) #No diamonds that are fair and more than $30000

diamonds |> 
  filter(cut == "Ideal" & price < 2000) # diamonds of ideal cut are less than

#5

diamonds |> 
  mutate(price_cat1 = if_else(price < 500, "low", "high"), .keep = "used")
