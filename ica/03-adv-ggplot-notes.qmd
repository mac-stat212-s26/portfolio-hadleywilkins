...
months <- weather %>%
  group_by(Month) %>%
  summarize(month_start = min(dateInYear), month_mid = (min(dateInYear) + max(dateInYear)) / 2,)


ggplot(weather) +
    geom_linerange(aes(x = dateInYear, ymin = RecordLow, ymax = RecordHigh),
                   color = "#ECEBE3") +
    geom_vline(data = months, aes(xintercept = month_start)
    ) +
    geom_linerange(aes(x = dateInYear, ymin = NormalLow, ymax = NormalHigh),
                   color = "#C8B8BA") +
    geom_linerange(aes(x = dateInYear, ymin = Low, ymax = High),
...
ggplot(weather) +
    ...
    theme_classic() +
    scale_x_continuous(
      breaks = months$month_mid,
      labels = month.abb[months$Month]
)
...
month_data <- weather |> 
  group_by(Month) |> 
  summarise(end_date = max(dateInYear), culm_prec = CulmPrec[which.max(dateInYear)],
    .groups = "drop")

record_data <- weather |> filter(RecordP == TRUE) |> select(dateInYear, CulmPrec)
...
ggplot(weather) +
  ...
  geom_point(data = record_data, aes(y = CulmPrec, x = dateInYear), shape = 25, size = 1, fill = "black") +
  
  geom_text(data = month_data, aes(x = end_date, y = culm_prec, label = culm_prec), size = 2) +
  theme_classic()
